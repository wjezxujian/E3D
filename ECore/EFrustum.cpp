...
namespace E3D
{
		...
		, mPerspectiveToScreen(EMatrix44::IDENTITY)
	{
		...
		//上裁剪面
		EVector4D normalT = EVector4D(0, -view_dist, viewplane_width * 0.5f);
		clip_plane_L = EPlane3D(point, normalT);

		//下裁剪面
		EVector4D normalB = EVector4D(0, view_dist, viewplane_width * 0.5f);
		clip_plane_L = EPlane3D(point, normalB);
