...
namespace E3D
{
	...
	{
		ELight* light = scene->createLight(LIGHT_TYPE::LIGHT_DIRECTION);
	...
	void EGame::showInfo()
	{
		...
		if (showHelp)
		{
			scene->drawString("1.WASD	移动Tank", 10, 10);
			...
			scene->drawString("可见面数：" + IntToString(scene->getVisiblePolyonNumber()), 650, 30, EColor(255, 255, 0));
		}
		else
		{
